...
def anagrams(string, array)
  ...
  array.select do |word|
    if word.chars.sort.join("") == string
      arr << word.chars.join("")
    end
  end
  arr
...
def ordered_vowel_word?(word)
  vowels = 'aeiou'.chars
  vowels_in_word = word.chars.select do |letters|
    letters
    vowels.include?(letters) 
  end
...
def ordered_vowels(words)
  # vowels = 'aeiou'.chars
  # words = words.join(" ")
  # vowels_in_word = words.chars.select do |letters|
  #   letters
  #   vowels.include?(letters) 
  # end
...
def no_repeat_years(first_year, last_year)
  array = (first_year..last_year).to_a
  hold_years = []
  array.each_with_index do |word, idx|
    word = word.to_s.split("")
    unless word[0] == word[1] || word[1] == word[2] || word[2] == word[3] || word[3] == word[4] || word[0] == word[2] || word[0] == word[3] || word[0] == word[4] || word[1] == word[3] || word[1] == word[4] || word[2] == word[0] || word[2] == word[4] || word[3] == word[1] || word[3] == word[0]
      word = word.join("").to_i
      hold_years << word
    end
  end 
  p hold_years.flatten
end
