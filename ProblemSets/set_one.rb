...
# # A Slippery Number is a number that has 3 as a factor or has 5 as a factor, but does *not* have both as factors. For example, 6 is a Slippery Number, but 30 is not. Write a function that given an N, returns an array of the first N Slippery numbers.
...
def slippery_numbers(n)
  array_of_nums = []
  number = 1
  until array_of_nums.length == n
    if is_slippery?(number)   # slippery numbers 
...
def is_slippery?(number)
  if number % 3 == 0 || number % 5 == 0  
    if number % 15 != 0     # a slippery number
      return true
    end
  else
    return false
...
def magic_numbers(count)
  array_of_nums = []
  number = 1
  until array_of_nums.length == count
    if is_magic_number?(number)   # slippery numbers 
      array_of_nums << number
    end
    number+=1
  end
...
def is_magic_number?(number)
  number = number.to_s.split("").map {|i| Integer(i) }
  sum = number.reduce(:+)
  if sum == 7
    return true
  else
